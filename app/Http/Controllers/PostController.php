...
use App\Models\Post;         //insert the post model
use App\Models\Category;     //insert the category model


class PostController extends Controller
{
    private $post;     //property
    private $category;

    public function __construct(Post $post, Category $category){
        $this->post = $post;        //$this->post:poject
        $this->category = $category;
    }

    #This method is going to retrieve all the categories in categories table
    # and display the categories into the create.blade.php
    ...
    public function store(Request $request){
        ...
        #2. prepare the view (create.blade.php) so that it will properly display the error message when the vialidation encounters a message

        #3. Save the post into the database table
        $this->post->user_id    = Auth::user()->id; //John Smith --> id of 1
        $this->post->image      = 'data:image/' . $request->image->extension() . ';base64,' . base64_encode(file_get_contents($request->image)); //data:image/->label
    ...
    public function edit($id)
    {
        ...
        // If the user is not the OWNER of the post, then redirect to the homepage. user can't edit the post
        ...
        foreach ($post->categoryPost as $category_post) {   //$post is from line 91
    ...
    // update method is use to do the actual update of the resouse
    public function update(Request $request, $id)
    {
        ...
        // Use the relationship Post::categoryPost() to select the recoad related to a post
